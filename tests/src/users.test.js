...
describe('users module', () => {
  ...
  it('createUser() should throw an error for invalid user fields', () => {
    expect(() => createUser({ id: '1', name: 'John Doe' })).toThrow('Invalid user fields: name, email, and role must be strings');
  });
  ...
  it('updateUser() should update a user by ID', () => {
    ...
    expect(updateUser('1', updates)).toEqual({
      ...user,
      ...updates,
      createdAt: expect.any(String)
    });
  ...
  it('getUsersByRole() should return users with the specified role', () => {
    ...
    createUser(user1);
    createUser(user2);
    expect(getUsersByRole('admin')).toEqual([{
      ...user1,
      createdAt: expect.any(String)
    }]);
  });

  // Scenario: throws an error for invalid role type
  it('getUsersByRole() should throw an error for non-string role', () => {
    expect(() => getUsersByRole(123)).toThrow('Role must be a string');
  });
  ...
  it('searchUsers() should return users matching the query', () => {
    ...
    createUser(user1);
    createUser(user2);
    expect(searchUsers('Jane')).toEqual([{
      ...user2,
      createdAt: expect.any(String)
    }]);
  });
